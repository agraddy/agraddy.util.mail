...
var server = smtp.createServer(function(req, res) {
    ...
    } else if(req.to == 'headers@example.com') {
		tap.assert.equal(req.from, 'from@example.com', 'The from field should be set.');

		req.pipe(writable).on('finish', function() {
			message_id = content.match(/Message-Id: (.*)\r\n/)[1];
			fs.readFile(path.join('fixtures', 'headers.eml'), function(err, data) {
				data = data.toString().replace(/\r?\n/g, '\r\n');
				data = data.replace('{{date}}', date);
				data = data.replace('{{message_id}}', message_id);
				// May fail from time to time if the date is a second off
...
function dkim() {
	...
	date =  new Date().toUTCString().replace('GMT', '+0000');
	// Eventually need to add an html argument after the text argument
    mod('dkim@example.com', 'from@example.com', 'Test Subject', 'This is a test message that is long enough that the content should wrap to the next line.', '', {dkim: {private_key_location: './fixtures/private_key.pem', d: 'example.com', s: 'alpha', h: ['From']}}, function(err) {
		tap.assert.equal(err, null, 'Should be equal.');
		reject();
	});
}
...
function headers() {
	...
	date =  new Date().toUTCString().replace('GMT', '+0000');
	// Eventually need to add an html argument after the text argument
    mod('headers@example.com', 'from@example.com', 'Test Subject', 'This is a test message that is long enough that the content should wrap to the next line.', {'Reply-To': 'reply@example.com'}, function(err) {
		tap.assert.equal(err, null, 'Should be equal.');
        // Make sure any new test email addresses are added to config/index.js
		end();
	});
}
