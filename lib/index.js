...
var mod = function(to, from, subject, text, html, dkim, cb) {
	if(typeof html == 'function') {
		cb = html;
	} else if(typeof dkim == 'function') {
		cb = dkim;
	}
	...
	function start() {
		...
		if(typeof dkim == 'object') {
			dkim_private_key_location = dkim.private_key_location;
			delete dkim.private_key_location;
			mail.unshift(sign(mail.join('').slice(0, -3), dkim, dkim_private_key_location));
		}
